...
async def get_or_create_contact(phone_number: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = supabase.table('contacts').select('*').eq('phone', phone_number).execute()
        ...
        if response.data:
            ...
            insert_response = supabase.table('contacts').insert(
                {'phone': phone_number}, returning="representation"
            ).execute()
...
async def log_conversation(contact_id: str, client_id: str, transcript: Any, summary: Optional[str] = None):
    """
...
async def update_contact_name(phone_number: str, name: str) -> bool:
    ...
    try:
        response = supabase.table('contacts').update({'name': name}).eq('phone', phone_number).execute()
        
        # The V1 client might not give a good indicator of success on update,
        # so we check if data was returned or if there was no error.
        # A successful update on a match returns a list with the updated dict.
        # A successful update on NO match returns an empty list.
        if response.data or not getattr(response, 'error', True):
            logger.info(f"Successfully updated name for {phone_number}.")
            return True
        else:
            error_message = getattr(response, 'error', 'Unknown error during update')
            logger.error(f"Failed to update name for {phone_number}: {error_message}")
            return False
            
    except APIError as e:
        logger.error(f"Supabase API error in update_contact_name: {e.body}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error in update_contact_name: {e}")
        return False